...
const AUTH_PLUGIN_LEGACY = 'Legacy_Auth',
 	  AUTH_PLUGIN_SRP = 'Srp';
const AUTH_PLUGIN_SRP256 = 'Srp256';

const AUTH_PLUGIN_LIST = [AUTH_PLUGIN_SRP256, AUTH_PLUGIN_SRP, AUTH_PLUGIN_LEGACY];
const AUTH_PLUGIN_SRP_LIST = [AUTH_PLUGIN_SRP256, AUTH_PLUGIN_SRP];
const LEGACY_AUTH_SALT = '9z';
const WIRE_CRYPT_DISABLE = 0;
const WIRE_CRYPT_ENABLE = 1;

exports.AUTH_PLUGIN_LEGACY = AUTH_PLUGIN_LEGACY;
exports.AUTH_PLUGIN_SRP_LIST = AUTH_PLUGIN_SRP_LIST;
exports.LEGACY_AUTH_SALT = LEGACY_AUTH_SALT;
                ...
                if (r === op_cond_accept || r === op_accept_data) {
                    ...
                    if (is_authenticated === 0) {
                    	if (cnx.options.pluginName && cnx.options.pluginName !== accept.pluginName) {
                    		doError(new Error('Server rejected plugin : ' + cnx.options.pluginName + ', but support : ' + accept.pluginName), callback);
						}
                    	...
                    	if (AUTH_PLUGIN_SRP_LIST.indexOf(accept.pluginName) !== -1) {
                            ...
                            if (accept.pluginName === AUTH_PLUGIN_LEGACY) {
                                ...
                                logger.info('Unknown auth plugin : ' + accept.pluginName);
                                return cb(new Error('Unknown auth plugin : ' + accept.pluginName));
                            }
